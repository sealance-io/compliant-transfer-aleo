...
const CONFIG = {
  ...
  ignorePatterns: ["node_modules", "dist", "build", ".git", "package-lock.json", "yarn.lock"],
};
...
async function processDirectory(directoryPath) {
  try {
    ...
    for (const entry of entries) {
      ...
      if (CONFIG.ignorePatterns.some(pattern => fullPath.includes(pattern) || entry.name.includes(pattern))) {
        continue;
      }
...
function findBestExtensionForImport(importPath, baseDir) {
  ...
  let absolutePath = path.isAbsolute(importPath) ? importPath : path.resolve(baseDir, importPath);
