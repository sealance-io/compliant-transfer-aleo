...
export async function upgradeProgram(programName: string, privateKey: string) {
  ...
  console.log(
    `cd artifacts/leo/${programName} && leo upgrade --broadcast ${isDevnet ? "--devnet" : ""} --private-key ${privateKey} --yes --endpoint ${endpoint} --network ${networkName} --blocks-to-check 20 --max-wait 30`,
  );
