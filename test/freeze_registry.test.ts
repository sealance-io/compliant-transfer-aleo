...
describe("test freeze_registry program", () => {
  ...
  test(
    ...
    async () => {
      ...
      let rejectedTx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, true, 1, 0n, root);
      await expect(rejectedTx.wait()).rejects.toThrow();

      const tx = await freezeRegistryContract.initialize(BLOCK_HEIGHT_WINDOW);
      await tx.wait();
      const isAccountFreezed = await freezeRegistryContract.freeze_list(ZERO_ADDRESS);
      ...
      rejectedTx = await freezeRegistryContract.initialize(BLOCK_HEIGHT_WINDOW);
      await expect(rejectedTx.wait()).rejects.toThrow();
    },
    timeout,
  );
  ...
  test(
    ...
    async () => {
      ...
      rejectedTx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, false, 1, currentRoot, root);
      await expect(rejectedTx.wait()).rejects.toThrow();

      // Cannot update the root if the previous root is incorrect
      rejectedTx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, false, 1, 0n, root);
      await expect(rejectedTx.wait()).rejects.toThrow();

      let tx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, true, 1, currentRoot, root);
      await tx.wait();
      let isAccountFreezed = await freezeRegistryContract.freeze_list(freezedAccount);
      let freezedAccountByIndex = await freezeRegistryContract.freeze_list_index(1);
      ...
      rejectedTx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, false, 2, root, root);
      await expect(rejectedTx.wait()).rejects.toThrow();

      // Cannot freeze a frozen account
      rejectedTx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, true, 1, root, root);
      await expect(rejectedTx.wait()).rejects.toThrow();

      tx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, false, 1, root, root);
      await tx.wait();
      ...
      tx = await freezeRegistryContractForAdmin.update_freeze_list(freezedAccount, true, 1, root, root);
      await tx.wait();
      isAccountFreezed = await freezeRegistryContract.freeze_list(freezedAccount);
      freezedAccountByIndex = await freezeRegistryContract.freeze_list_index(1);
      lastIndex = await freezeRegistryContract.freeze_list_last_index(FREEZE_LIST_LAST_INDEX);
      ...
      freezedAccountByIndex = await freezeRegistryContract.freeze_list_index(2);
      lastIndex = await freezeRegistryContract.freeze_list_last_index(FREEZE_LIST_LAST_INDEX);
  ...
  test(
    ...
    async () => {
      const rejectedTx = await freezeRegistryContractForFreezedAccount.update_block_height_window(BLOCK_HEIGHT_WINDOW);
      await expect(rejectedTx.wait()).rejects.toThrow();
