...
import { Block_listContract } from "../artifacts/js/block_list";
import { Merkle_tree8Contract } from "../artifacts/js/merkle_tree8";

const mode = ExecutionMode.SnarkExecute;
const mrkl = new Merkle_tree8Contract({ mode });
const contract = new Block_listContract({ mode });
...
describe('blocklist test', () => {
 /* test(`deploy merkle_tree8`, async () => {
    const tx = await mrkl.deploy();
    await tx.wait();
  }, 10000000)

  test(`deploy blocklist`, async () => {
...
*/
  test(`happy path`, async () => {
    const tx = await contract.build_sorted_tree([
      "aleo1s3ws5tra87fjycnjrwsjcrnw2qxr8jfqqdugnf0xzqqw29q9m5pqem2u4t",
      "aleo193cgzzpr5lcwq6rmzq4l2ctg5f4mznead080mclfgrc0e5k0w5pstfdfps",
      "aleo194vjp7nt6pwgpruw3kz5fk5kvj9ur6sg2f4k84fqu6cpgq5xhvrs7emymc",
      "aleo1wkyn0ax8nhftfxn0hkx8kgh46yxqla7tzd6z77jhcf5wne6z3c9qnxl2l4",
    ...
    const tx2 = await contract.siblings_path(result, 2);
    const [merkle_proof2] =  await tx2.wait();
    console.log(merkle_proof2)

    const tx3 = await contract.siblings_path(result, 3);
    ...
    const tx5 = await contract.siblings_path(result, 4);
    const [merkle_proof4] =  await tx5.wait();
    console.log(merkle_proof4)

    // the siblings indeces are not adjusted
  ...
  test(`remove address`, async () => {
    ...
    const tx2 = await contract.remove_address("aleo16k94hj5nsgxpgnnk9u6580kskgucqdadzekmlmvccp25frwd8qgqvn9p9t", tree[14]);
    await tx2.wait();
  }, 10000000)
  ...
  test(`add address`, async () => {
    ...
    const tx2 = await contract.add_address("aleo16k94hj5nsgxpgnnk9u6580kskgucqdadzekmlmvccp25frwd8qgqvn9p9t", tree[14]);
    await tx2.wait();
  }, 10000000)
