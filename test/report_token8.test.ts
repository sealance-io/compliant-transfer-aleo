...
import { Sealed_report_token_v8Contract } from "../artifacts/js/sealed_report_token_v8";
import { stringToBigInt } from "../lib/Conversion";
import { decryptToken } from "../artifacts/js/leo2js/sealed_report_token_v8";
import { Token } from "../artifacts/js/types/sealed_report_token_v8";
...
const reportTokenContract = new Sealed_report_token_v8Contract({
  mode,
  privateKey: deployerPrivKey,
});
const reportTokenContractForAdmin = new Sealed_report_token_v8Contract({
  mode,
  privateKey: adminPrivKey,
});
const reportTokenContractForAccount = new Sealed_report_token_v8Contract({
  mode,
  privateKey: accountPrivKey,
});
const reportTokenContractForMinter = new Sealed_report_token_v8Contract({
  mode,
  privateKey: minterPrivKey,
});
const reportTokenContractForBurner = new Sealed_report_token_v8Contract({
  mode,
  privateKey: burnerPrivKey,
});
const reportTokenContractForSupplyManager = new Sealed_report_token_v8Contract({
  mode,
  privateKey: supplyManagerPrivKey,
});
const reportTokenContractForSpender = new Sealed_report_token_v8Contract({
  mode,
  privateKey: spenderPrivKey,
});
const reportTokenContractForFreezedAccount = new Sealed_report_token_v8Contract({
  mode,
  privateKey: freezedAccountPrivKey,
});
const merkleTreeContract = new Merkle_tree8Contract({
  mode,
  privateKey: deployerPrivKey,
});
...
describe("test sealed_report_token program", () => {
  ...
  test(
    ...
    async () => {
      ...
      senderMerkleProof = [
        getSiblingPath(tree, senderLeafIndices[0], MAX_TREE_SIZE8),
        getSiblingPath(tree, senderLeafIndices[1], MAX_TREE_SIZE8),
      ];
      recipientMerkleProof = [
        getSiblingPath(tree, recipientLeafIndices[0], MAX_TREE_SIZE8),
        getSiblingPath(tree, recipientLeafIndices[1], MAX_TREE_SIZE8),
      ];
      freezedAccountMerkleProof = [
        getSiblingPath(tree, freezedAccountLeafIndices[0], MAX_TREE_SIZE8),
        getSiblingPath(tree, freezedAccountLeafIndices[1], MAX_TREE_SIZE8),
      ];
    },
    timeout,
  );
