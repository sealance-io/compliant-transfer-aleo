...
import { Ticket } from "../artifacts/js/types/sealed_standalone_token";
import { decryptTicket } from "../artifacts/js/leo2js/sealed_standalone_token";
import { Sealed_standalone_tokenContract } from "../artifacts/js/sealed_standalone_token";
...
const standaloneTokenContract = new Sealed_standalone_tokenContract({
  mode,
  privateKey: deployerPrivKey,
});
const standaloneTokenContractForAdmin = new Sealed_standalone_tokenContract({
  mode,
  privateKey: adminPrivKey,
});
const standaloneTokenContractForFreezeListManager = new Sealed_standalone_tokenContract({
  mode,
  privateKey: freezeListManagerPrivKey,
});
const standaloneTokenContractForAccount = new Sealed_standalone_tokenContract({
  mode,
  privateKey: accountPrivKey,
});
const standaloneTokenContractForMinter = new Sealed_standalone_tokenContract({
  mode,
  privateKey: minterPrivKey,
});
const standaloneTokenContractForBurner = new Sealed_standalone_tokenContract({
  mode,
  privateKey: burnerPrivKey,
});
const standaloneTokenContractForSupplyManager = new Sealed_standalone_tokenContract({
  mode,
  privateKey: supplyManagerPrivKey,
});
const standaloneTokenContractForSpender = new Sealed_standalone_tokenContract({
  mode,
  privateKey: spenderPrivKey,
});
const standaloneTokenContractForFrozenAccount = new Sealed_standalone_tokenContract({
  mode,
...
describe("test sealed_standalone_token program", () => {
  ...
  test(`test update_admin_address`, async () => {
    let tx = await standaloneTokenContractForAdmin.update_role(frozenAccount, ADMIN_INDEX);
    await tx.wait();
    let adminRole = await standaloneTokenContract.roles(ADMIN_INDEX);
    expect(adminRole).toBe(frozenAccount);

    tx = await standaloneTokenContractForFrozenAccount.update_role(adminAddress, ADMIN_INDEX);
    await tx.wait();
    adminRole = await standaloneTokenContract.roles(ADMIN_INDEX);
    expect(adminRole).toBe(adminAddress);

    tx = await standaloneTokenContractForFrozenAccount.update_role(frozenAccount, ADMIN_INDEX);
  ...
  test(`test update_freeze_list_manager`, async () => {
    let tx = await standaloneTokenContractForAdmin.update_role(freezeListManager, FREEZE_LIST_MANAGER_INDEX);
    await tx.wait();
    const freezeListManagerRole = await standaloneTokenContract.roles(FREEZE_LIST_MANAGER_INDEX);
    expect(freezeListManagerRole).toBe(freezeListManager);

    tx = await standaloneTokenContractForFrozenAccount.update_role(frozenAccount, FREEZE_LIST_MANAGER_INDEX);
    await expect(tx.wait()).rejects.toThrow();
  });
  ...
  test(`test initialize`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, true, 1, 0n, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
    ...
    rejectedTx = await standaloneTokenContract.initialize(name, symbol, decimals, maxSupply, BLOCK_HEIGHT_WINDOW);
    await expect(rejectedTx.wait()).rejects.toThrow();
  });
  ...
  test(`test update_freeze_list`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForFrozenAccount.update_freeze_list(
      adminAddress,
      true,
      1,
      currentRoot,
    ...
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, false, 1, currentRoot, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // Cannot update the root if the previous root is incorrect
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, false, 1, 0n, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
    ...
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, false, 2, root, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // Cannot freeze a frozen account
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, true, 1, root, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    tx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, false, 1, root, root);
    await tx.wait();
    isAccountFrozen = await standaloneTokenContract.freeze_list(frozenAccount);
    frozenAccountByIndex = await standaloneTokenContract.freeze_list_index(1);
    lastIndex = await standaloneTokenContract.freeze_list_last_index(FREEZE_LIST_LAST_INDEX);

    expect(isAccountFrozen).toBe(false);
    expect(frozenAccountByIndex).toBe(ZERO_ADDRESS);
    expect(lastIndex).toBe(1);

    tx = await standaloneTokenContractForFreezeListManager.update_freeze_list(frozenAccount, true, 1, root, root);
    await tx.wait();
    isAccountFrozen = await standaloneTokenContract.freeze_list(frozenAccount);
    frozenAccountByIndex = await standaloneTokenContract.freeze_list_index(1);
    lastIndex = await standaloneTokenContract.freeze_list_last_index(FREEZE_LIST_LAST_INDEX);
    ...
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(randomAddress, true, 10, root, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // Cannot freeze an account when the freezed list index is already taken
    rejectedTx = await standaloneTokenContractForAdmin.update_freeze_list(randomAddress, true, 2, root, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
  });

  test(`test update_supply_role`, async () => {
    // Non admin user cannot update minter role
    let rejectedTx = await standaloneTokenContractForAccount.update_supply_role(minter, MINTER_ROLE);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // Non admin user cannot update burner role
    rejectedTx = await standaloneTokenContractForAccount.update_supply_role(burner, BURNER_ROLE);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // Non admin user cannot update supply manager role
    rejectedTx = await standaloneTokenContractForAccount.update_supply_role(supplyManager, SUPPLY_MANAGER_ROLE);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // Non admin user cannot update none role
    rejectedTx = await standaloneTokenContractForAccount.update_supply_role(account, SUPPLY_MANAGER_ROLE);
    await expect(rejectedTx.wait()).rejects.toThrow();

    let tx = await standaloneTokenContractForAdmin.update_supply_role(minter, MINTER_ROLE);
    await tx.wait();
    let role = await standaloneTokenContract.supply_roles(minter);
    expect(role).toBe(MINTER_ROLE);

    tx = await standaloneTokenContractForAdmin.update_supply_role(burner, BURNER_ROLE);
    await tx.wait();
    role = await standaloneTokenContract.supply_roles(burner);
    expect(role).toBe(BURNER_ROLE);

    tx = await standaloneTokenContractForAdmin.update_supply_role(supplyManager, SUPPLY_MANAGER_ROLE);
    await tx.wait();
    role = await standaloneTokenContract.supply_roles(supplyManager);
    expect(role).toBe(SUPPLY_MANAGER_ROLE);

    tx = await standaloneTokenContractForAdmin.update_supply_role(account, NONE_ROLE);
    await tx.wait();
    role = await standaloneTokenContract.supply_roles(account);
  ...
  test(`test mint_private`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForAccount.mint_private(account, amount * 20n);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // a burner cannot mint private assets
    rejectedTx = await standaloneTokenContractForBurner.mint_private(account, amount * 20n);
    await expect(rejectedTx.wait()).rejects.toThrow();
    ...
    tx = await standaloneTokenContractForMinter.mint_private(frozenAccount, amount * 20n);
    ...
    tx = await standaloneTokenContractForSupplyManager.mint_private(account, amount * 20n);
    await tx.wait();
  });

  test(`test mint_public`, async () => {
    // a regular user cannot mint public assets
    let rejectedTx = await standaloneTokenContractForAccount.mint_public(account, amount * 20n);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // a burner cannot mint public assets
    rejectedTx = await standaloneTokenContractForBurner.mint_public(account, amount * 20n);
    await expect(rejectedTx.wait()).rejects.toThrow();

    let tx = await standaloneTokenContractForAdmin.mint_public(account, amount * 20n);
    await tx.wait();
    let balance = await standaloneTokenContract.balances(account);
    expect(balance).toBe(amount * 20n);

    tx = await standaloneTokenContractForMinter.mint_public(frozenAccount, amount * 20n);
    await tx.wait();
    balance = await standaloneTokenContract.balances(frozenAccount);
    expect(balance).toBe(amount * 20n);

    tx = await standaloneTokenContractForSupplyManager.mint_public(account, amount * 20n);
    await tx.wait();
    balance = await standaloneTokenContract.balances(account);
    expect(balance).toBe(amount * 40n);
  });

  test(`test mint_public_gen_ticket`, async () => {
    // a regular user cannot mint public assets
    let rejectedTx = await standaloneTokenContractForAccount.mint_public_gen_ticket(account, amount, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // a burner cannot mint public assets
    rejectedTx = await standaloneTokenContractForBurner.mint_public_gen_ticket(account, amount, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // It's impossible to mint and generate ticket if the recipient is on the freeze list
    rejectedTx = await standaloneTokenContractForAdmin.mint_public_gen_ticket(frozenAccount, amount, root);
    await expect(rejectedTx.wait()).rejects.toThrow();
    // It's impossible to mint and generate ticket if the root is incorrect
    rejectedTx = await standaloneTokenContractForAdmin.mint_public_gen_ticket(account, amount, 0n);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const tx = await standaloneTokenContractForAdmin.mint_public_gen_ticket(account, amount, root);
    const [encryptedTicket] = await tx.wait();
    const ticket = decryptTicket(encryptedTicket, accountPrivKey);
  ...
  test(`test burn_private`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForAccount.burn_private(accountRecord, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    let mintTx = await standaloneTokenContractForAdmin.mint_private(adminAddress, amount);
    ...
    let burnTx = await standaloneTokenContractForAdmin.burn_private(adminRecord, amount);
    ...
    mintTx = await standaloneTokenContractForAdmin.mint_private(burner, amount);
    ...
    burnTx = await standaloneTokenContractForBurner.burn_private(burnerRecord, amount);
    ...
    mintTx = await standaloneTokenContractForAdmin.mint_private(supplyManager, amount);
    ...
    burnTx = await standaloneTokenContractForSupplyManager.burn_private(supplyManagerRecord, amount);
  ...
  test(`test burn_public`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForAccount.burn_public(account, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // A minter user cannot burn public assets
    rejectedTx = await standaloneTokenContractForMinter.burn_public(account, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);
    let tx = await standaloneTokenContractForAdmin.burn_public(account, amount);
    await tx.wait();
    let balance = await standaloneTokenContract.balances(account);
    expect(balance).toBe(previousAccountPublicBalance - amount);

    tx = await standaloneTokenContractForBurner.burn_public(account, amount);
    await tx.wait();
    balance = await standaloneTokenContract.balances(account);
    expect(balance).toBe(previousAccountPublicBalance - amount * 2n);

    tx = await standaloneTokenContractForSupplyManager.burn_public(account, amount);
    await tx.wait();
    balance = await standaloneTokenContract.balances(account);
  ...
  test(`test update_block_height_window`, async () => {
    const rejectedTx = await standaloneTokenContractForAccount.update_block_height_window(BLOCK_HEIGHT_WINDOW);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const tx = await standaloneTokenContractForAdmin.update_block_height_window(BLOCK_HEIGHT_WINDOW);
    await tx.wait();
  });

  test(`test transfer_public`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    let rejectedTx = await standaloneTokenContractForFrozenAccount.transfer_public(recipient, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // If the recipient is frozen account it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForAccount.transfer_public(frozenAccount, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);
    const previousRecipientPublicBalance = await standaloneTokenContract.balances(recipient, 0n);

    const tx = await standaloneTokenContractForAccount.transfer_public(recipient, amount);
    await tx.wait();

    const accountPublicBalance = await standaloneTokenContract.balances(account);
    const recipientPublicBalance = await standaloneTokenContract.balances(recipient);
  ...
  test(`test transfer_public_gen_ticket`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    let rejectedTx = await standaloneTokenContractForFrozenAccount.transfer_public_gen_ticket(recipient, amount, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // If the recipient is frozen account it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForAccount.transfer_public_gen_ticket(frozenAccount, amount, root);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // It's impossible to mint and generate ticket if the root is incorrect
    rejectedTx = await standaloneTokenContractForAccount.transfer_public_gen_ticket(recipient, amount, 0n);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);
    const previousRecipientPublicBalance = await standaloneTokenContract.balances(recipient, 0n);

    const tx = await standaloneTokenContractForAccount.transfer_public_gen_ticket(recipient, amount, root);
    const [encryptedTicket] = await tx.wait();
    const ticket = decryptTicket(encryptedTicket, recipientPrivKey);
    expect(ticket.owner).toBe(recipient);
    expect(ticket.freeze_list_root).toBe(root);
  ...
  test(`test transfer_public_as_signer`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    let rejectedTx = await standaloneTokenContractForFrozenAccount.transfer_public_as_signer(recipient, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // If the recipient is frozen account it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForAccount.transfer_public_as_signer(frozenAccount, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);
    const previousRecipientPublicBalance = await standaloneTokenContract.balances(recipient, 0n);

    const tx = await standaloneTokenContractForAccount.transfer_public_as_signer(recipient, amount);
    await tx.wait();
  ...
  test(`test transfer_from_public`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForSpender.transfer_from_public(account, recipient, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    let approveTx = await standaloneTokenContractForAccount.approve_public(spender, amount);
    await approveTx.wait();
    let unapproveTx = await standaloneTokenContractForAccount.unapprove_public(spender, amount);
    await unapproveTx.wait();

    // If the sender approve and then unapprove the spender the transaction will fail
    rejectedTx = await standaloneTokenContractForSpender.transfer_from_public(account, recipient, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // approve the spender
    approveTx = await standaloneTokenContractForAccount.approve_public(spender, amount);
    await approveTx.wait();
    approveTx = await standaloneTokenContractForFrozenAccount.approve_public(spender, amount);
    await approveTx.wait();

    // If the sender is frozen account it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForSpender.transfer_from_public(frozenAccount, recipient, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    // If the recipient is frozen account it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForSpender.transfer_from_public(account, frozenAccount, amount);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);
    const previousRecipientPublicBalance = await standaloneTokenContract.balances(recipient, 0n);

    const tx = await standaloneTokenContractForSpender.transfer_from_public(account, recipient, amount);
    await tx.wait();
  ...
  test(`test transfer_from_public_to_private`, async () => {
    // If the sender didn't approve the spender the transaction will fail
    let rejectedTx = await standaloneTokenContractForSpender.transfer_from_public_to_private(
      account,
      recipient,
      amount,
      investigatorAddress,
    ...
    // If the sender approve and then unapprove the spender the transaction will fail
    rejectedTx = await standaloneTokenContractForSpender.transfer_from_public_to_private(
      account,
      recipient,
      amount,
      investigatorAddress,
    ...
    const tx = await standaloneTokenContractForSpender.transfer_from_public_to_private(
      account,
      recipient,
      amount,
      investigatorAddress,
  ...
  test(`test transfer_public_to_priv`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    let rejectedTx = await standaloneTokenContractForFrozenAccount.transfer_public_to_private(
      recipient,
      amount,
      investigatorAddress,
    );
    ...
    // If the investigator address is wrong it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForAccount.transfer_public_to_private(recipient, amount, ZERO_ADDRESS);
    await expect(rejectedTx.wait()).rejects.toThrow();

    const previousAccountPublicBalance = await standaloneTokenContract.balances(account);

    const tx = await standaloneTokenContractForAccount.transfer_public_to_private(
      recipient,
      amount,
      investigatorAddress,
    );
  ...
  test(`test transfer_private`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    await expect(
      standaloneTokenContractForFrozenAccount.transfer_private(
        recipient,
        amount,
        accountRecord,
        frozenAccountMerkleProof,
    ...
    // If the investigator address is wrong it's impossible to send tokens
    const rejectedTx = await standaloneTokenContractForAccount.transfer_private(
      recipient,
      amount,
      accountRecord,
      senderMerkleProof,
  ...
  test(`test transfer_priv_to_public`, async () => {
    // If the sender is frozen account it's impossible to send tokens
    await expect(
      standaloneTokenContractForFrozenAccount.transfer_private_to_public(
        recipient,
        amount,
        frozenAccountRecord,
        frozenAccountMerkleProof,
    ...
    // If the recipient is frozen account it's impossible to send tokens
    let rejectedTx = await standaloneTokenContractForAccount.transfer_private_to_public(
      frozenAccount,
      amount,
      accountRecord,
      senderMerkleProof,
    ...
    // If the investigator address is wrong it's impossible to send tokens
    rejectedTx = await standaloneTokenContractForAccount.transfer_private_to_public(
      recipient,
      amount,
      accountRecord,
      senderMerkleProof,
    ...
    const tx = await standaloneTokenContractForAccount.transfer_private_to_public(
      recipient,
      amount,
      accountRecord,
      senderMerkleProof,
  ...
  test(`test get_ticket`, async () => {
    ...
    await expect(standaloneTokenContractForFrozenAccount.get_ticket(frozenAccountMerkleProof)).rejects.toThrow();
    ...
    const rejectedTx = await standaloneTokenContractForAccount.get_ticket(IncorrectSenderMerkleProof);
    await expect(rejectedTx.wait()).rejects.toThrow();
  ...
  test(`test transfer with ticket`, async () => {
    // If the investigator address is incorrect the transaction will be rejected
    let rejectedTransferPrivateTx = await standaloneTokenContractForAccount.transfer_private_with_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
    ...
    // If the recipient is frozen the transaction will be rejected
    let rejectedTransferPrivToPublicTx = await standaloneTokenContractForAccount.transfer_priv_to_public_ticket(
      frozenAccount,
      amount,
      accountRecord,
      ticket,
    ...
    await expect(rejectedTransferPrivToPublicTx.wait()).rejects.toThrow();
    // If the investigator address is incorrect the transaction will be rejected
    rejectedTransferPrivToPublicTx = await standaloneTokenContractForAccount.transfer_priv_to_public_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
    ...
    let transferPrivToPublicTx = await standaloneTokenContractForAccount.transfer_priv_to_public_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
    ...
    let updateFreezeListTx = await standaloneTokenContractForAdmin.update_freeze_list(
      frozenAccount,
      false,
      1,
      root,
    ...
    let updateBlockHeightWindowTx = await standaloneTokenContractForAdmin.update_block_height_window(1);
    await updateBlockHeightWindowTx.wait();

    rejectedTransferPrivateTx = await standaloneTokenContractForAccount.transfer_private_with_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
    ...
    updateFreezeListTx = await standaloneTokenContractForAdmin.update_freeze_list(frozenAccount, true, 1, 1n, root);
    await updateFreezeListTx.wait();
    updateBlockHeightWindowTx = await standaloneTokenContractForAdmin.update_block_height_window(BLOCK_HEIGHT_WINDOW);
    await updateBlockHeightWindowTx.wait();

    transferPrivateTx = await standaloneTokenContractForAccount.transfer_private_with_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
    ...
    transferPrivToPublicTx = await standaloneTokenContractForAccount.transfer_priv_to_public_ticket(
      recipient,
      amount,
      accountRecord,
      ticket,
  ...
  test(`test old root support`, async () => {
    ...
    let rejectedTx = await standaloneTokenContractForAccount.transfer_private(
      recipient,
      amount,
      accountRecord,
    ...
    const updateFreezeListTx = await standaloneTokenContractForAdmin.update_freeze_list(
      frozenAccount,
      false,
      1,
      root,
    ...
    const newRoot = await standaloneTokenContract.freeze_list_root(CURRENT_FREEZE_LIST_ROOT_INDEX);
    const oldRoot = await standaloneTokenContract.freeze_list_root(PREVIOUS_FREEZE_LIST_ROOT_INDEX);
    ...
    const updateBlockHeightWindowTx = await standaloneTokenContractForAdmin.update_block_height_window(1);
    await updateBlockHeightWindowTx.wait();

    // The transaction failed because the old root is expired
    rejectedTx = await standaloneTokenContractForAccount.transfer_private(
      recipient,
      amount,
      accountRecord,
      senderMerkleProof,
