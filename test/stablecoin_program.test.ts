...
const tokenContractForAccount = new Stablecoin_programContract({
  mode,
  privateKey: accountPrivKey,
});
const tokenContractForMinter = new Stablecoin_programContract({
  mode,
  privateKey: minterPrivKey,
});
const tokenContractForBurner = new Stablecoin_programContract({
  mode,
  privateKey: burnerPrivKey,
});
const tokenContractForSupplyManager = new Stablecoin_programContract({
  mode,
  privateKey: supplyManagerPrivKey,
});
const tokenContractForSpender = new Stablecoin_programContract({
  mode,
  privateKey: spenderPrivKey,
});
const tokenContractForFrozenAccount = new Stablecoin_programContract({
  mode,
  privateKey: frozenAccountPrivKey,
});
const tokenContractForPauser = new Stablecoin_programContract({
  mode,
      ...
      for (const block of blockRange) {
        ...
        for (const tx of block.transactions) {
          ...
          for (const transition of tx.transaction?.execution?.transitions ?? []) {
            if (
              transition.program === "stablecoin_program.aleo" &&
              transition.outputs &&
